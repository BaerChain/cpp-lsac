...
std::pair<bool, std::string> wallet::ToolTransaction::sign_trx_from_json(std::string json_str, std::string& transaction_hash) {
    ...
    try {
                    ...
                    switch (type) {
                        ...
                        case changeMiner:{
                            ...
                            for(auto &nodeMsg : op_obj["m_agreeMsg"].get_array()){
                                agreeMsgs.emplace_back(Signature(nodeMsg.get_str()));
                            }
        ...
        if (obj.count("keys")) {
            ...
            _pair.second = "not find the keys_item in json_data";
        }
        ...
        for (auto &t : trx_datas) {
            if (keys.count(t.from)) {
                ...
                //exit(1);
                _pair.second = "not find the from_address key";
            }
        }
    }
    catch (...){
        _pair.second = "some error about json_data";
    }*/
    ...
    try {
        ...
        for (auto &t : trx_datas) {
            if (keys.count(t.from)) {
                ...
                std::cout << "please input address: " << t.from << " private key.";
                _pair.second = "not find the addrss key...";
            }
        }
    }
    catch (...) {
