...
wallet::WalletServer::WalletServer(dev::SafeHttpServer& server, std::string _send_url) : AbstractServer<WalletServer>(server), m_send_url(_send_url)
{
    ...
    this->bindAndAddMethod(Procedure("sign_transaction", PARAMS_BY_NAME, JSON_STRING, "param", JSON_OBJECT, NULL), &WalletServer::sign_transaction);

    this->bindAndAddMethod(
        Procedure("sign_transaction_send", PARAMS_BY_NAME, JSON_STRING, "param", JSON_OBJECT, NULL), &WalletServer::sign_transaction_send);

    //    this->bindAndAddMethod(Procedure("new_address", PARAMS_BY_POSITION, jsonrpc::JSON_OBJECT, "param1", jsonrpc::JSON_STRING, NULL),
    //    &WalletServer::new_address);

    this->bindAndAddMethod(Procedure("new_address", PARAMS_BY_POSITION, JSON_OBJECT, "param1", JSON_STRING, NULL), &WalletServer::new_address);

    this->bindAndAddMethod(Procedure("sign_data", PARAMS_BY_NAME, JSON_STRING, "param", JSON_OBJECT, NULL), &WalletServer::sign_data);
}
