...
Json::Value HttpWallet::sign_transaction(Json::Value const& _param)
{
    Json::Value respone;
    try
    {
...
Json::Value HttpWallet::sign_transaction_send(Json::Value const& _param)
{
...
Json::Value HttpWallet::new_address(std::string const& _param)
{
    Json::Value respone;
    try
    {
        ...
        if (create_number > 0)
        {
            for (int i = 0; i < create_number; i++)
            {
                ...
                value["new-address"] = dev::jsToNewAddress(key_pair.address());
