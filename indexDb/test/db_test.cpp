...
//        o.buy_type = get_random_type() ? dx::order_buy_type::all_price : dx::order_buy_type::only_price;
    ...
    BOOST_AUTO_TEST_CASE(get_result_orders_by_address_test6) {
        bbfs::path cur_dir = bbfs::current_path();
        cur_dir /= bbfs::path("data");
        cur_dir /= bbfs::unique_path();
        ...
        try {
            ...
            Address addr("0000000000000000000000000000000000000003");
            {
                auto ret = db.get_result_orders_by_news(100);
                std::cout << "ret size " << ret.size() << std::endl;
                for(auto itr : ret){
                    if(itr.acceptor == addr || itr.sender == addr){
                        std::cout << "acceptor " << toHex(itr.acceptor) << " sender : " << toHex(itr.sender) << " time:" << itr.create_time << std::endl;
                    }
                }
            ...
            {
                std::cout << " time: " << t.elapsed() << "  size: " << test.size() << std::endl;
                auto ret = db.get_result_orders_by_address(addr, 30, 36, 50);
