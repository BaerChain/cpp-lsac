...
{
public:
    explicit EVM(bvmc_instance* _instance) noexcept;

    ~EVM() { bvmc_destroy(m_instance); }
    ...
    /// Translate the EVMSchedule to BVMC revision.
    static bvmc_revision toRevision(EVMSchedule const& _schedule);
...
class BVMC : public EVM, public VMFace
{
public:
    explicit BVMC(bvmc_instance* _instance) : EVM(_instance) {}
