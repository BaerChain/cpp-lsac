...
void Block::resetCurrent(int64_t _timestamp) {
	...
	m_state.setBlockNumber(m_previousBlock.number());
    m_precommit = m_state;
...
bool Block::sync(BlockChain const &_bc, h256 const &_block, BlockHeader const &_bi) {
    ...
    if (bi == m_currentBlock) {
        ...
        if (m_state.db().lookup(bi.stateRoot()).empty())  // TODO: API in State for this?
        {
            ...
            cwarn << "Try rescuing the database by running: brc --rescue";
...
bool Block::sealBlock(bytesConstRef _header) {
    ...
    if (block_header.hash(flag) != m_currentBlock.hash(flag)) {
        //        cdebug << BrcYellow << "bytesConstRef _header :";
...
void Block::testNetAddBalance(BlockHeader const& _currInfo) {
    if (config::chainId() == TESTCHAINID && _currInfo.number() == 34570000) {
