...
std::ostream& dev::brc::operator<<(std::ostream& _out, SyncStatus const& _sync)
{
    ...
    _out << "state: " << BrcdChainCapability::stateName(_sync.state) << " ";
...
void BlockChainSync::onPeerStatus(BrcdChainPeer const& _peer)
{
    RecursiveGuard l(x_sync);
    DEV_INVARIANT_CHECK;
...
void BlockChainSync::onPeerNewBlock(NodeID const& _peerID, RLP const& _r)
{
    ...
    cwarn << " from id : " << _peerID << " time:" << utcTimeMilliSec() << " get block time " << (utcTimeMilliSec() - info.timestamp()) << " ms " << " height " << info.number() ;
    unsigned blockNumber = static_cast<unsigned>(info.number());
