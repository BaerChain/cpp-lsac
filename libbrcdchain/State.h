...
struct Change
{
    ...
    Change(Address const& _addr, std::unordered_map<h256, bytes> const& _map) : kind(StorageByte), address(_addr) { storageByte = _map; }

    Change(Address const& _addr, std::vector<h256> const& _v) : kind(DeleteStorgaeByte), address(_addr) { deleteStorageByte = _v; }
    ...
    Change(Address const& _addr, bytes const& _oldCode) : kind(Code), address(_addr), oldCode(_oldCode) {}
    ...
    Change(Address const& _addr, VoteSnapshot const& _vote) : kind(NewVoteSnapshot), address(_addr) { vote_snapshot = _vote; }
    Change(Address const& _addr, CouplingSystemfee const& _fee) : kind(CoupingSystemFeeSnapshot), address(_addr) { feeSnapshot = _fee; }
    Change(Kind _kind, Address const& _addr, PollData const& p_data) : kind(_kind), address(_addr) { poll_data = p_data; }
    Change(Kind _kind, Address const& _addr, std::pair<u256, int64_t> const& value) : kind(_kind), address(_addr) { create_record = value; }
    Change(Kind _kind, Address const& _addr, std::vector<PollData> const& poll) : kind(_kind), address(_addr) { minners = poll; }
    Change(Kind _kind, Address const& _addr, ReceivedCookies const& _received) : kind(_kind), address(_addr) { received = _received; }
    ...
    Change(Kind _kind, Address const& _addr, std::pair<Address, Address> const& _map) : kind(_kind), address(_addr) { mapping = _map; }

    Change(Kind _kind, Address const& _addr, std::map<h256, CancelOrder> const& _orders) : kind(_kind), address(_addr) { cancelOrders = _orders; }
    Change(Kind _kind, Address const& _addr, std::map<Address, u256> const& _map) : kind(_kind), address(_addr) { minerGasPrice = _map; }
};
...
public:
    ...
    explicit State(u256 const& _accountStartNonce) : State(_accountStartNonce, OverlayDB(), ex::exchange_plugin(), BaseState::Empty) {}

    /// Basic state object from database.
    /// Use the default when you already have a database and you just want to make a State
    /// object which uses it. If you have no preexisting database then set BaseState to
    /// something other than BaseState::PreExisting in order to prepopulate the Trie.
    explicit State(u256 const& _accountStartNonce, OverlayDB const& _db, ex::exchange_plugin const& _exdb, BaseState _bs = BaseState::PreExisting);
    ...
    /// @warning This is slowslowslow. Don't use it unless you want to lock the object for
    /// seconds or minutes at a time.
    /// @throws InterfaceNotSupported if compiled without BRC_FATDB.
    std::unordered_map<Address, u256> addresses() const;

    /// @returns the map with maximum _maxResults elements containing hash->addresses and the
    /// next address hash. This method faster then addresses() const;
    ...
    /// Check if the account exists in the state and is non empty (nonce > 0 || balance > 0 ||
    /// code nonempty). These two notions are equivalent after EIP158.
    ...
    //
    void pendingOrder(Address const& _addr, u256 _pendingOrderNum, u256 _pendingOrderPrice, h256 _pendingOrderHash, ex::order_type _pendingOrderType,
        ex::order_token_type _pendingOrderTokenType, ex::order_buy_type _pendingOrderBuyType, int64_t _nowTime);

    // void pendingOrders(Address const& _addr, int64_t _nowTime, h256 _pendingOrderHash,
    // std::vector<std::shared_ptr<transationTool::operation>> const& _ops, u256 &_exCookieNum =
    // u256(0), u256 &_exBRCNum = u256(0));
     std::pair<u256, u256> pendingOrders(Address const& _addr, int64_t _nowTime, h256 _pendingOrderHash,
    ...
    void freezeAmount(Address const& _addr, u256 _pendingOrderNum, u256 _pendingOrderPrice, ex::order_type _pendingOrderType,
        ex::order_token_type _pendingOrderTokenType, ex::order_buy_type _pendingOrderBuyType);

    void pendingOrderTransfer(Address const& _from, Address const& _to, u256 _toPendingOrderNum, u256 _toPendingOrderPrice,
        ex::order_type _pendingOrderType, ex::order_token_type _pendingOrderTokenType, ex::order_buy_type _pendingOrderBuyTypes);
    ...
    bool verifyExchangeOrderExits(h256 const& _hash, int64_t const& _time, u256 const& _price, ex::order_type const& _type, Address const& _sender);
    ...
    void transferAutoEx(std::vector<std::shared_ptr<transationTool::operation>> const& _ops, h256 const& _trxid, int64_t _timeStamp,
        u256 const& _baseGas, int64_t const& _blockNum);
...
public:
    ...
    std::vector<Address> verifyGetAuthorityCookiesAddrs(Address const& _addr, transationTool::getRootKeyType const& _type, bool isRoot);
...
private:
    ...
    void updateAddressSet(Address const& _from, Address const& _changeAddr, bool _isDel, transationTool::getRootKeyType const& _type);
...
public:
    ...
    /// Get the original value of a storage position of an account (before modifications saved
    /// in account cache).
    /// @returns 0 if no account exists at that address.
    ...
    /// @returns EmptySHA3 if no account exists at that address or if there is no code
    ...
    void setBlockNumber(int64_t value)
    {
        ...
        m_curr_number = m_block_number + 1;
    }
...
private:
    ...
    void calculateMasterNodeDividend(
        std::map<Address, std::pair<u256, u256>>& _masterNode, u256 const& _totalDividendAmount, u256 const& _allMinerVotes);
    void votingAccountDividend(
        std::set<Address> const& _voteAddrs, std::map<Address, std::pair<u256, u256>> _masterNodeDividendInfo, int64_t _blockNum);

public:

    // dividend code
    void testDividend(std::set<Address> const& _dividendAddrs, int64_t _blockNum);
    void testCalculateMasterNodeDividend(
        std::map<Address, u256> const& _masterNodeVotes, std::map<Address, u256> const& _masterNodeDividend, u256 const& _totalVotes);
    void testVotingAccountDividend(std::set<Address> const& _voteAddrs, std::map<Address, u256> const& _masterNodeDividends,
    ...
    /// @returns true when normally halted; false when exceptionally halted; throws when
    /// internal VM exception occurred.
    ...
    /// Our address cache. This stores the states of each address that has (or at least might
    /// have) been changed.
