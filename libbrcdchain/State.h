...
struct Change
{
    ...
    public:
        ...
        explicit State(
            u256 const& _accountStartNonce, OverlayDB const& _db, ex::exchange_plugin const& _exdb, BaseState _bs = BaseState::PreExisting);
        ...
        /// This will change the state accordingly.
        void executeBlockTransactions(
            Block const& _block, unsigned _txCount, LastBlockHashesFace const& _lastHashes, SealEngineFace const& _sealEngine);
        ...
        //
        void pendingOrder(Address const& _addr, u256 _pendingOrderNum, u256 _pendingOrderPrice, h256 _pendingOrderHash,
            ex::order_type _pendingOrderType, ex::order_token_type _pendingOrderTokenType, ex::order_buy_type _pendingOrderBuyType, int64_t _nowTime);
        ...
        std::pair<u256, u256> pendingOrders(Address const& _addr, int64_t _nowTime, int64_t blockHeight, h256 _pendingOrderHash,
        ...
        bool verifyExchangeOrderExits(
            h256 const& _hash, int64_t const& _time, u256 const& _price, ex::order_type const& _type, Address const& _sender);
        // Json::Value newExorderAllGet();
        Json::Value newExorderGetByType(uint8_t _order_type);
        void newGetSellExChangeOrder(
            int64_t const& _time, u256 const& _price, boost::optional<std::pair<sellOrder::iterator, sellOrder::iterator>>& _p);
        ...
        void transferAutoEx(std::vector<std::shared_ptr<transationTool::operation>> const& _ops, h256 const& _trxid, int64_t _timeStamp,
            int64_t height, u256 const& _baseGas);

        std::pair<Address, Address> minerMapping(Address const& addr);

    public:
        void transferAuthorityControl(
            Address const& _from, std::vector<std::shared_ptr<transationTool::operation>> const& _ops, EnvInfo const& _envinfo);
