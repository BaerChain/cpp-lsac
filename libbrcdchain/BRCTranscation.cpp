...
void dev::brc::BRCTranscation::verifyTransactions(std::vector<bytes> const& _bs,
    Address const& _from, std::vector<std::shared_ptr<transationTool::operation>>& _operations)
{
    u256 _totalBrc = 0;
    ...
    Address const& _form, Address const& _to, size_t _type, const u256& _transcationNum)
{
    if (_type <= dev::brc::TranscationEnum::ETranscationNull ||
        ...
        (_type == dev::brc::TranscationEnum::EBRCTranscation && _transcationNum == 0))
    {
        BOOST_THROW_EXCEPTION(BrcTranscationField() << errinfo_comment(
                                  "the brc transaction's type is error:" + toString(_type)));
    }
    ...
    if (_type == dev::brc::TranscationEnum::EBRCTranscation)
    {
        if (_form == _to)
        {
            BOOST_THROW_EXCEPTION(
                BrcTranscationField() << errinfo_comment(" cant't transfer brc to me"));
        }
...
void dev::brc::BRCTranscation::verifyPendingOrders(Address const& _form, u256 _total_cost,
    ex::exchange_plugin& _exdb, int64_t _nowTime, u256 _transcationGas, h256 _pendingOrderHash,
    std::vector<std::shared_ptr<transationTool::operation>> const& _ops, int64_t _blockHeight)
{
    u256 total_brc = 0;
    ...
    for (auto const& val : _ops)
    {
        ...
        if (!pen)
            BOOST_THROW_EXCEPTION(VerifyPendingOrderFiled() << errinfo_comment(
                                      std::string("Pending order type is incorrect!")));
        ...
        if (_type == order_type::null_type || _token_type == order_token_type::BRC ||
            (_buy_type == order_buy_type::only_price &&
                (_type == order_type::sell || _type == order_type::buy) &&
        ...
        {
            BOOST_THROW_EXCEPTION(VerifyPendingOrderFiled() << errinfo_comment(std::string(
                                      "Pending order type and parameters are incorrect")));
        }

        if ((_type != ex::order_type::sell && _type != ex::order_type::buy) ||
            _token_type != ex::order_token_type::FUEL ||
            (_buy_type != ex::order_buy_type::all_price &&
                _buy_type != ex::order_buy_type::only_price))
        {
            BOOST_THROW_EXCEPTION(VerifyPendingOrderFiled() << errinfo_comment(std::string(
                                      "Pending order type and parameters are incorrect")));
        }
        ...
        {
            BOOST_THROW_EXCEPTION(
                VerifyPendingOrderFiled() << errinfo_comment(std::string(
                    "Pending order accuracy cannot be greater than 4 digits accuracy")));
        }
        ...
        if (_type == order_type::buy)
        {
            if (_buy_type == order_buy_type::only_price)
            {
                ...
                {
                    BOOST_THROW_EXCEPTION(VerifyPendingOrderFiled() << errinfo_comment(std::string(
                                              "buy Cookie only_price :Address BRC < Num * Price")));
                }
                if (total_brc < BUYCOOKIELIMIT)
                {
                    BOOST_THROW_EXCEPTION(
                        VerifyPendingOrderFiled() << errinfo_comment(std::string(
                            "Limit orders to buy cookies can not be less than 10000 BRC")));
                }
            }
            else if (_buy_type == order_buy_type::all_price)
            {
                ...
                {
                    BOOST_THROW_EXCEPTION(VerifyPendingOrderFiled() << errinfo_comment(std::string(
                                              "buy Cookie all_price :Address BRC < Num * Price")));
                }
            }
        }
        else if (_type == order_type::sell)
        {
            if (_buy_type == order_buy_type::only_price || _buy_type == order_buy_type::all_price)
            {
                ...
                {
                    BOOST_THROW_EXCEPTION(
                        VerifyPendingOrderFiled() << errinfo_comment(
        ...
        if (_buy_type == order_buy_type::all_price)
        {
            ...
            {
                BOOST_THROW_EXCEPTION(VerifyPendingOrderFiled() << errinfo_comment(std::string(
                                          "There is no order for the corresponding order pool!")));
            }
        }
            ...
            _buy_type == order_buy_type::all_price)
        {
            ...
            ex_order _order = {_pendingOrderHash, _form, _pendingOrderPrice * PRICEPRECISION,
                _pendingOrderNum, _pendingOrderNum, _nowTime, (order_type)_type,
                (order_token_type)_token_type, (order_buy_type)_buy_type};
    ...
    for (auto _val : _verfys)
    {
        try
        {
            ...
            for (auto it : _retV)
            {
                if (it.type == order_type::buy && it.token_type == order_token_type::FUEL &&
                    it.buy_type == order_buy_type::all_price)
                {
            ...
            {
                BOOST_THROW_EXCEPTION(VerifyPendingOrderFiled() << errinfo_comment(
                                          std::string("pendingorderFailed : The exchanged cookies "
                                                      "are not enough to pay the commission!")));
            }
        }
        catch (const boost::exception& e)
        {
            ...
            BOOST_THROW_EXCEPTION(VerifyPendingOrderFiled() << errinfo_comment(std::string(
                                      "pendingorderFailed : buy BRC allprice is failed!")));
        }
        catch (...)
        {
            BOOST_THROW_EXCEPTION(VerifyPendingOrderFiled() << errinfo_comment(std::string(
                                      "pendingorderFailed : buy BRC allprice unkonwn failed!")));
        }
    }
}


void dev::brc::BRCTranscation::verifyCancelPendingOrders(ex::exchange_plugin& _exdb, Address _addr,
    std::vector<std::shared_ptr<transationTool::operation>> const& _ops, int64_t _blockNum)
{
    std::vector<h256> _HashV;
    for (auto const& val : _ops)
    {
        std::shared_ptr<transationTool::cancelPendingorder_operation> can_order =
            std::dynamic_pointer_cast<transationTool::cancelPendingorder_operation>(val);
        if (!can_order)
            BOOST_THROW_EXCEPTION(CancelPendingOrderFiled()
                                  << errinfo_comment(std::string("Pendingorder type is error!")));
        if (can_order->m_hash == h256(0))
            BOOST_THROW_EXCEPTION(CancelPendingOrderFiled()
                                  << errinfo_comment(std::string("Pendingorder hash cannot be 0")));
            ...
            else
            {
                ...
                if (orderCancel.m_id != _it)
                {
                    ...
                    BOOST_THROW_EXCEPTION(CancelPendingOrderFiled() << errinfo_comment(
                                              std::string("Pendingorder hash cannot be find")));
                }
                /// verify order in newExDB
                auto ret = m_state.verifyExchangeOrderExits(_it, orderCancel.m_time,
                    orderCancel.m_price, (ex::order_type)orderCancel.m_type, _addr);
                if (!ret)
                {
                    cwarn << "Pendingorder cannot be find in ExDB";
                    BOOST_THROW_EXCEPTION(CancelPendingOrderFiled() << errinfo_comment(
                                              std::string("Pendingorder cannot be find in ExDB")));
                }
            }
        }
    }
    catch (const boost::exception& e)
    {
        ...
        BOOST_THROW_EXCEPTION(CancelPendingOrderFiled() << errinfo_comment(
                                  std::string("This order does not exist in the trading pool")));
    }
    ...
    if (config::changeExchange() > _blockNum)
    {
        if (_resultV.size() == 0)
        {
            BOOST_THROW_EXCEPTION(CancelPendingOrderFiled() << errinfo_comment(std::string(
                                      "This order does not exist in the trading pool  . vertiy")));
        }
        for (auto val : _resultV)
        {
            ...
            {
                BOOST_THROW_EXCEPTION(
                    CancelPendingOrderFiled() << errinfo_comment(std::string(
                        "This order is not the same as the transaction sponsor account")));
            }
        }
    }
}
...
void dev::brc::BRCTranscation::verifyreceivingincomeChanegeMiner(dev::Address const& _from,
    ...
    dev::brc::transationTool::dividendcycle _type, dev::brc::EnvInfo const& _envinfo,
    ...
    if (_envinfo.number() >= config::newChangeHeight())
    {
        ...
        if (!(miner_mapping.first == Address() || miner_mapping.second == _from))
        {
            ...
            BOOST_THROW_EXCEPTION(receivingincomeFiled() << errinfo_comment(
                                      std::string("miner_mapping can not to receive")));
        }
...
void dev::brc::BRCTranscation::verifyreceivingincome(dev::Address const& _from,
    std::vector<std::shared_ptr<transationTool::operation>> const& _ops,
    dev::brc::transationTool::dividendcycle _type, dev::brc::EnvInfo const& _envinfo,
    dev::brc::DposVote const& _vote)
{
    for (auto const& _it : _ops)
    {
        ...
        {
            BOOST_THROW_EXCEPTION(receivingincomeFiled()
...
void dev::brc::BRCTranscation::verifyBlockFeeincome(
    dev::Address const& _from, const dev::brc::EnvInfo& _envinfo, const dev::brc::DposVote& _vote)
{
    std::pair<uint32_t, Votingstage> _pair = config::getVotingCycle(_envinfo.number());
    if (_pair.second == Votingstage::ERRORSTAGE)
    {
        BOOST_THROW_EXCEPTION(receivingincomeFiled() << errinfo_comment(
                                  std::string("There is currently no income to receive")));
    }

    auto a = m_state.account(_from);
    if (!a)
    {
        BOOST_THROW_EXCEPTION(receivingincomeFiled() << errinfo_comment(std::string(
                                  "The account that receives the income does not exist")));
    }
    ...
    {
        BOOST_THROW_EXCEPTION(receivingincomeFiled() << errinfo_comment(
                                  std::string("Unable to get system account instance")));
    }
    ...
    {
        BOOST_THROW_EXCEPTION(receivingincomeFiled() << errinfo_comment(std::string(
                                  "no votedataHistory: There is currently no income to receive")));
    }
    ...
    std::map<u256, std::map<Address, u256>>::const_iterator _voteIt =
        _voteSnapshot.m_voteDataHistory.find(_numberofround - 1);
    if (_voteIt == _voteSnapshot.m_voteDataHistory.end())
    {
        ...
        {
            BOOST_THROW_EXCEPTION(receivingincomeFiled() << errinfo_comment(
                                      std::string("isMainNode fasle :The node that this account "
                                                  "votes does not have a super node")));
        }
    }
    else
    {
        ...
        {
            BOOST_THROW_EXCEPTION(receivingincomeFiled() << errinfo_comment(
                                      std::string(" voteDataSnapshot end: The node that this "
                                                  "account votes does not have a super node")));
        }
    }
}

void dev::brc::BRCTranscation::verifyPdFeeincome(
    dev::Address const& _from, int64_t _blockNum, dev::brc::DposVote const& _vote)
{
    ...
    for (int i = (int)_numofRounds; i < _pair.first; i++)
    {
        if (_voteSnapshot.m_voteDataHistory.count(i - 1) && _map.count(i))
        {
            ...
            {
                if (_voteSnapshot.m_voteDataHistory[i - 1].count(val.m_addr) &&
                    _voteSnapshot.m_voteDataHistory[i - 1][val.m_addr] != 0)
                {
                    is_received = true;
                    return;
                }
    ...
    {
        BOOST_THROW_EXCEPTION(receivingincomeFiled() << errinfo_comment(
                                  std::string("This account does not meet the redemption fee")));
    }
}

void dev::brc::BRCTranscation::verifyTransferAutoEx(const dev::Address& _from,
    const std::vector<std::shared_ptr<dev::brc::transationTool::operation>>& _op,
    u256 const& _baseGas, h256 const& _trxid, dev::brc::EnvInfo const& _envinfo)
{
    int64_t const& _timeStamp = _envinfo.timestamp();
    if ((_envinfo.number() < config::autoExTestNetHeight() &&
            _envinfo.header().chain_id() == 0x1) ||
        (_envinfo.number() < config::autoExHeight() && _envinfo.header().chain_id() == 0xb))
    {
        BOOST_THROW_EXCEPTION(
            transferAutoExFailed() << errinfo_comment(std::string(
                "Transfer automatic exchange fee function has not yet reached the opening time")));
    }
    for (auto const& _opIt : _op)
    {
        std::shared_ptr<transationTool::transferAutoEx_operation> _autoExop =
            std::dynamic_pointer_cast<transationTool::transferAutoEx_operation>(_opIt);
        if (_autoExop->m_from != _from)
        {
            BOOST_THROW_EXCEPTION(
                transferAutoExFailed() << errinfo_comment(std::string(
                    "Initiating transaction address is inconsistent with operation address")));
        }
        ...
        if (_autoExop->m_autoExType == transationTool::transferAutoExType::Balancededuction)
        {
            if (_autoExop->m_transferNum + _autoExop->m_autoExNum > m_state.BRC(_from))
            {
                BOOST_THROW_EXCEPTION(
                    transferAutoExFailed() << errinfo_comment(std::string(
                        "The number of accounts BRC is not enough to complete the transaction")));
            }
        }
        else if (_autoExop->m_autoExType == transationTool::transferAutoExType::Transferdeduction)
        {
            ...
            {
                BOOST_THROW_EXCEPTION(transferAutoExFailed() << errinfo_comment(
                                          std::string("The number of automatic redemptions is "
                                                      "greater than the number of transfers")));
            }
            if (_autoExop->m_transferNum > m_state.BRC(_from))
            {
                BOOST_THROW_EXCEPTION(
                    transferAutoExFailed() << errinfo_comment(std::string(
                        "The number of transfer BRC is greater than the account balance")));
            }
        }
        else
        {
            BOOST_THROW_EXCEPTION(transferAutoExFailed() << errinfo_comment(
                                      std::string("Automatically redeem unknown exceptions")));
        }

        std::vector<ex::result_order> _result;
        ex::ex_order _order = {_trxid, _from, _autoExop->m_autoExNum * PRICEPRECISION, u256(0),
            u256(0), _timeStamp, ex::order_type::buy, ex::order_token_type::FUEL,
        ...
        {
            BOOST_THROW_EXCEPTION(transferAutoExFailed() << errinfo_comment(
                                      std::string("Automatically redeemed cookies are insufficient "
                                                  "to pay for the fuel cost of the transaction")));
        }
    }
}
    ...
    std::vector<std::shared_ptr<transationTool::operation>> const& _ops, EnvInfo const& _envinfo)
{
    for (auto const& _it : _ops)
    {
        std::shared_ptr<transationTool::authority_operation> _op =
            std::dynamic_pointer_cast<transationTool::authority_operation>(_it);
        if (!_op)
        {
            BOOST_THROW_EXCEPTION(transferAuthotityControlFailed()
                                  << errinfo_comment(std::string("authority_operation is error")));
        }
        // cwarn <<"verify"<< _op->m_childAddress << " weight:"<<(int)_op->m_weight << "
        // permiss:"<<(int)_op->m_permissions; verify address
        if (_from == _op->m_childAddress)
        {
            BOOST_THROW_EXCEPTION(transferAuthotityControlFailed() << errinfo_comment(
                                      std::string("rootAddress and childAddress can't be same")));
        }
        if (_op->m_weight != 0 && _op->m_permissions == transationTool::op_type::null)
        {
            BOOST_THROW_EXCEPTION(transferAuthotityControlFailed() << errinfo_comment(
                                      std::string("the Invalid controlTransacion params")));
        }
        // verify the premassion
        if ((authority::PermissionsType)_op->m_permissions != authority::PermissionsType::null)
            authority::getPermissionsTypeByTransactionType(
                (transationTool::op_type)_op->m_permissions);

        // verify the weight
        if (!authority::checkWeight(_op->m_weight))
        {
            BOOST_THROW_EXCEPTION(transferAuthotityControlFailed() << errinfo_comment(std::string(
                                      "the value of weight invalid, range of [0,100]")));
        }
        // verify child account
        auto _data = m_state.getDataByKeyAddress(
            _from, _op->m_childAddress, dev::brc::transationTool::getRootKeyType::ChildDataKey);
        AccountControl _control = AccountControl{_data};
        if (!_control.m_authority.empty() && _control.m_childAddress != _op->m_childAddress)
        {
            BOOST_THROW_EXCEPTION(transferAuthotityControlFailed() << errinfo_comment(std::string(
                                      "Invalid storage about:" + toJS(_op->m_childAddress))));
        }
        if (!_control.updateAuthority((authority::PermissionsType)_op->m_permissions, _op->m_weight)
                 .first)
        {
            BOOST_THROW_EXCEPTION(transferAuthotityControlFailed() << errinfo_comment(
                                      std::string("childAdress Conterol_value is not changed")));
        }
        ...
        if (_control.m_authority.empty())
            isDel = true;
        auto childAddrs =
            m_state.getAddressesByRootKey(_from, transationTool::getRootKeyType::ChildAddrKey);
        auto ret = std::find(childAddrs.begin(), childAddrs.end(), _op->m_childAddress);
        if (isDel)
        {
            if (ret == childAddrs.end())
                BOOST_THROW_EXCEPTION(
                    transferAuthotityControlFailed() << errinfo_comment(std::string(
                        "rootAddress not contains childAddress:" + toJS(_op->m_childAddress))));
        }
        else
        {
            if (ret == childAddrs.end())
            {
                ...
                if (childAddrs.size() >= authority::ChildAddressNum)
                    BOOST_THROW_EXCEPTION(
                        transferAuthotityControlFailed() << errinfo_comment(
                            std::string("the number of childAddress can't out:" +
                                        std::to_string(authority::ChildAddressNum))));
            }
        }
        ...
        if (_op->m_childAddress != _control.m_childAddress && isDel)
        {
            ...
            BOOST_THROW_EXCEPTION(transferAuthotityControlFailed()
                                  << errinfo_comment(std::string("the childAdress invalid")));
        }
    }
}
    ...
    Address const& _from, std::vector<std::shared_ptr<transationTool::operation>> const& _ops)
{
    for (auto const& val : _ops)
    {
        std::shared_ptr<transationTool::authorizeCookies_operation> _op =
            std::dynamic_pointer_cast<transationTool::authorizeCookies_operation>(val);
        if ((transationTool::authorizeCookieType)_op->m_authorizeType !=
                transationTool::authorizeCookieType::addCookieChild &&
            (transationTool::authorizeCookieType)_op->m_authorizeType !=
                transationTool::authorizeCookieType::deleteCookieChild)
        {
            BOOST_THROW_EXCEPTION(
                transferAuthorityUseCookieFailed() << errinfo_comment(std::string(
                    "transferAuthorityUseCookieFailed : authorize use cookie type is error")));
        }

        if (_from == _op->m_childAddress)
        {
            BOOST_THROW_EXCEPTION(transferAuthorityUseCookieFailed() << errinfo_comment(std::string(
                                      "transferAuthorityUseCookieFailed : The authorized address "
                                      "is the same as the authorized address")));
        }

        std::vector<Address> _childAddrs = m_state.verifyGetAuthorityCookiesAddrs(
            _from, transationTool::getRootKeyType::CookiesChildAddrKey, true);
        std::vector<Address> _rootAddrs = m_state.verifyGetAuthorityCookiesAddrs(
            _op->m_childAddress, transationTool::getRootKeyType::CookiesRootAddrKey, false);
        if ((transationTool::authorizeCookieType)_op->m_authorizeType ==
            transationTool::authorizeCookieType::addCookieChild)
        {
            if (std::find(_childAddrs.begin(), _childAddrs.end(), _op->m_childAddress) !=
                _childAddrs.end())
            {
                BOOST_THROW_EXCEPTION(
                    transferAuthorityUseCookieFailed() << errinfo_comment(std::string(
                        "transferAuthorityUseCookieFailed : This address has been authorized")));
            }
        ...
        else if ((transationTool::authorizeCookieType)_op->m_authorizeType ==
                 transationTool::authorizeCookieType::deleteCookieChild)
        {
            if (std::find(_childAddrs.begin(), _childAddrs.end(), _op->m_childAddress) ==
                _childAddrs.end())
            {
                BOOST_THROW_EXCEPTION(
                    transferAuthorityUseCookieFailed()
                    << errinfo_comment(std::string("transferAuthorityUseCookieFailed : The address "
                                                   "is not authorized and cannot be deleted")));
            }
            if (std::find(_rootAddrs.begin(), _rootAddrs.end(), _from) == _rootAddrs.end())
            {
                BOOST_THROW_EXCEPTION(
...
void dev::brc::BRCTranscation::verifyUseCookie(Address const& _rootAddr, Address const& _childAddr)
{
    ...
    std::vector<Address> _rootAddrs = m_state.verifyGetAuthorityCookiesAddrs(
        _childAddr, transationTool::getRootKeyType::CookiesRootAddrKey, false);
    ...
    if (!isPermission)
    {
        BOOST_THROW_EXCEPTION(
            VerifyPermissonTrxFailed() << errinfo_comment(
                std::string("Account does not have permission to use CookieAddress cookies")));
    }
}
...
bool dev::brc::BRCTranscation::isMainNode(
    dev::Address const& _addr, std::vector<PollData> const& _pollData)
{
    bool _status = false;
...
bool dev::brc::BRCTranscation::isMainNode(
    const std::map<Address, u256>& _voteData, const std::vector<dev::brc::PollData>& _pollData)
{
    bool _status = false;
...
bool dev::brc::BRCTranscation::isMainNode(const std::vector<dev::brc::PollData>& _voteData,
    const std::vector<dev::brc::PollData>& _pollData)
{
    bool _status = false;
