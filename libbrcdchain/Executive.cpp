...
u256 Executive::gasUsed() const
{
	if (m_envInfo.number() > config::modifyReciptGasHeight()) {
        ...
        if(m_envInfo.header().chain_id() == TESTCHAINID && m_envInfo.number() >= 32333372){
        ...
        else
        {
            ...
            for (auto val : _ops) {
				...
				if(is_verfy_cost && m_s.balance(m_t.sender()) < totalCost && _type != transationTool::transferAutoEx){
					...
					m_excepted = TransactionException::NotEnoughCash;
					std::string ex_info = "not enough Cookie to execute transaction will cost:" + toString(totalCost);
			...
			{
				m_excepted = TransactionException::NotEnoughCash;
				std::string ex_info = "not enough require cookie to execute transaction will cost:" + toString(totalCost);
...
bool Executive::finalize()
{
    if (m_ext)
    {
        ...
        m_ext->sub.refunds += m_ext->brcSchedule().suicideRefundGas * m_ext->sub.selfdestructs.size();
