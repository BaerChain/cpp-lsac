...
void Executive::initialize(Transaction const& _transaction, transationTool::initializeEnum _enum)
{
    ...
    if (!m_t.hasZeroSignature())
    {
            ...
            (m_envInfo.number() <= 1200157 && m_envInfo.header().chain_id() == 0x1))
        {
            ...
            {
                cdebug << "Sender: " << m_t.sender().hex() << " Invalid Nonce: Require " << nonceReq
                       << " Got " << m_t.nonce();
                m_excepted = TransactionException::InvalidNonce;
                BOOST_THROW_EXCEPTION(InvalidNonce()
                                      << RequirementError((bigint)nonceReq, (bigint)m_t.nonce())
...
void Executive::verifyTransactionOperation(u256 _totalCost, Address const& _from, std::vector<bytes>const& _ops, transationTool::op_type _baseType){
    ...
    if(m_t.from() == m_batch_params._rootAddress && m_s.getPermissionsTransfer(m_t.from(), m_batch_params._type)){
        std::string _str = "Invalid permission. the address permisssions:" + std::to_string(m_batch_params._type) +" is transfered";
        cwarn << _str;
...
bool Executive::call(CallParameters const& _p, u256 const& _gasPrice, Address const& _origin)
{
    ...
    else
    {
        ...
        switch (_type){
            ...
            case transationTool::op_type::pendingOrder:{
                m_s.pendingOrders(m_batch_params._rootAddress, m_envInfo.timestamp(), m_t.sha3(), m_batch_params._operation, m_envInfo.number());
                break;
            }
            case transationTool::op_type::cancelPendingOrder:{
