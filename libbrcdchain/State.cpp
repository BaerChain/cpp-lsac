...
std::pair<u256 ,u256> dev::brc::State::pendingOrders(Address const &_addr, int64_t _nowTime, h256 _pendingOrderHash,
    ...
    if (_set.size() > 0 && _blockNum < config::cancelAutoPendingOrderHeight()) {
        systemAutoPendingOrder(_set, _nowTime, _blockNum);
    }
...
{
    m_sellAccount->initOrder(m_db);
     m_sellAccount->sellExchangeGetIt(_price, _time, _p, m_db);
}
...
{
    m_sellAccount = account(dev::SellExchangeAddress);
    if(!m_sellAccount)
    {
        createAccount(dev::SellExchangeAddress, {0});
        m_sellAccount =  account(dev::SellExchangeAddress);
    }
    m_buyAccount = account(dev::BuyExchangeAddress);
    if(!m_buyAccount)
    {
...
dev::brc::commit(AccountMap const &_cache, SecureTrieDB<Address, DB> &_state, int64_t _block_number /*= 0*/) {
    ...
    AddressHash ret;
    // cwarn << "commit block number " << commitBlockNumber;
...
//                    }
                ...
                // cwarn << "insert:" << toHex(i.first) << " data:"<< s.out();
            }
            ret.insert(i.first);
        }
    //cwarn << "commit ....... end";
