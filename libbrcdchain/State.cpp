...
void State::pendingOrder(Address const& _addr, u256 _pendingOrderNum, u256 _pendingOrderPrice,
    ...
    ex::order_token_type _pendingOrderTokenType, ex::order_buy_type _pendingOrderBuyType,
    int64_t _nowTime)
{
    // add
...
void dev::brc::State::verifyChangeMiner(Address const& _from, EnvInfo const& _envinfo,
    std::vector<std::shared_ptr<transationTool::operation>> const& _ops)
{
    ...
    if (_ops.empty())
    {
        ...
        BOOST_THROW_EXCEPTION(
            ChangeMinerFailed() << errinfo_comment("changeMiner operation is empty"));
    }
    std::shared_ptr<transationTool::changeMiner_operation> pen =
        std::dynamic_pointer_cast<transationTool::changeMiner_operation>(_ops[0]);
    if (!pen)
    {
        cerror << "changeMiner  dynamic type field!";
        BOOST_THROW_EXCEPTION(
            ChangeMinerFailed() << errinfo_comment("Error Dynamic about changeMiner operation"));
    }
    if (_from != pen->m_before)
    {
        BOOST_THROW_EXCEPTION(
            ChangeMinerFailed() << errinfo_comment("The originator of the transaction is different "
                                                   "from the address of the replacement witness"));
    }
    ...
    if (!befor_miner)
    {
        ...
        BOOST_THROW_EXCEPTION(ChangeMinerFailed() << errinfo_comment(
                                  "not has the Acconut_miner :" + dev::toString(_from)));
    }
        ...
        std::find(canMiner.begin(), canMiner.end(), _from) == canMiner.end())
    {
        ...
        BOOST_THROW_EXCEPTION(
            ChangeMinerFailed() << errinfo_comment(dev::toString(_from) + " not is miner"));
    }
        ...
        std::find(canMiner.begin(), canMiner.end(), pen->m_after) != canMiner.end())
    {
        ...
        BOOST_THROW_EXCEPTION(
            ChangeMinerFailed() << errinfo_comment(dev::toString(_from) + " already is miner"));
    }
    ...
    if (!is_change)
    {
        ...
        BOOST_THROW_EXCEPTION(
            ChangeMinerFailed() << errinfo_comment("the sender has error mapping_address"));
    }
    ...
    {
        cwarn << " the maping after_addr has alreay mapping_addr :"
              << a_new->mappingAddress().first;
        BOOST_THROW_EXCEPTION(ChangeMinerFailed()
                              << errinfo_comment("the maping after_addr has alreay mapping_addr"));
    }
    ...
    if (_envinfo.number() > config::gasPriceHeight())
    {
        ...
        {
            BOOST_THROW_EXCEPTION(
                ChangeMinerFailed() << errinfo_comment("MinerGas SysAddress null"));
        }
        if (!a_gas->minerGasPrice().count(pen->m_before))
        {
            BOOST_THROW_EXCEPTION(
                ChangeMinerFailed() << errinfo_comment(
                    "the MinerGasPrice map not have address:" + dev::toString(pen->m_before)));
        }
        if (a_gas->minerGasPrice().count(pen->m_after))
        {
            BOOST_THROW_EXCEPTION(
                ChangeMinerFailed() << errinfo_comment(
                    "the MinerGasPrice map has contains address:" + dev::toString(pen->m_after)));
        }
    }
}

void dev::brc::State::changeMiner(
    std::vector<std::shared_ptr<transationTool::operation>> const& _ops)
{
    if (_ops.empty())
    {
    ...
    if (mapping_addr.first != Address() && mapping_addr.first != pen->m_before)
    {
        ...
        m_changeLog.emplace_back(
            Change::NewChangeMiner, mapping_addr.first, start_a->mappingAddress());
    ...
    if (befor_miner->mappingAddress().second != pen->m_before &&
        befor_miner->mappingAddress().first != pen->m_before)
    {
...
// void dev::brc::State::pendingOrders(Address const &_addr, int64_t _nowTime, h256
// _pendingOrderHash,
std::pair<u256, u256> dev::brc::State::pendingOrders(Address const& _addr, int64_t _nowTime,
    h256 _pendingOrderHash, std::vector<std::shared_ptr<transationTool::operation>> const& _ops,
    int64_t const& _blockNum)
{
    ...
    {
        std::shared_ptr<transationTool::pendingorder_opearaion> pen =
            std::dynamic_pointer_cast<transationTool::pendingorder_opearaion>(val);
        if (!pen)
        {
        ...
        u256 _price = 0;
        if (pen->m_Pendingorder_buy_type == order_buy_type::all_price &&
            pen->m_Pendingorder_type == order_type::buy)
        {
        ...
        ex_order _order = {_pendingOrderHash, _addr, _price, pen->m_Pendingorder_num,
            pen->m_Pendingorder_num, _nowTime, (order_type)pen->m_Pendingorder_type,
            (order_token_type)pen->m_Pendingorder_Token_type,
            (order_buy_type)pen->m_Pendingorder_buy_type};
        _v.push_back(_order);

        if (pen->m_Pendingorder_buy_type == order_buy_type::only_price)
        {
            if (pen->m_Pendingorder_type == order_type::buy &&
                pen->m_Pendingorder_Token_type == order_token_type::FUEL)
                total_free_brc +=
                    pen->m_Pendingorder_num * pen->m_Pendingorder_price / PRICEPRECISION;
    ...
    for (auto _val : _v)
    {
        ...
        for (uint32_t i = 0; i < _result_v.size(); ++i)
        {
            ...
            if (_result_order.type == order_type::buy &&
                _result_order.token_type == order_token_type::FUEL &&
                _result_order.buy_type == order_buy_type::all_price)
            {
            ...
            if (_result_order.buy_type == order_buy_type::only_price)
            {
                if (_result_order.type == order_type::buy &&
                    _result_order.token_type == order_token_type::FUEL)
                    total_free_brc -=
                        _result_order.amount * _result_order.old_price / PRICEPRECISION;
    ...
    {
        cerror << " this pindingOrder's free_balance or free_brc is error... balance:"
               << total_free_balance << " brc:" << total_free_brc;
        BOOST_THROW_EXCEPTION(pendingorderAllPriceFiled());
    }
...
void State::systemAutoPendingOrder(std::set<order_type> const& _set, int64_t _nowTime)
{
    ...
    for (auto it : _set)
    {
        if (it == order_type::buy)
        {
            ...
            ex_order _order = {_trHash, systemAddress, u256(BUYCOOKIE), _num, _num, _nowTime,
                order_type::buy, order_token_type::FUEL, order_buy_type::only_price};
            _v.push_back(_order);
        }
        else if (it == order_type::sell)
        {
            ...
            ex_order _order = {_trHash, systemAddress, u256(SELLCOOKIE), _num, _num, _nowTime,
                order_type::sell, order_token_type::FUEL, order_buy_type::only_price};
            _v.push_back(_order);
        }
    }
    ...
    for (auto _val : _v)
    {
        ...
        {
            pendingOrderTransfer(_result.sender, _result.acceptor, _result.amount, _result.price,
                _result.type, _result.token_type, _result.buy_type);
            ...
            if (_result.buy_type == order_buy_type::only_price)
            {
                ...
                else if (_result.type == order_type::sell &&
                         _result.token_type == order_token_type::FUEL)
...
void State::pendingOrderTransfer(Address const& _from, Address const& _to, u256 _toPendingOrderNum,
    ...
    ex::order_token_type _pendingOrderTokenType, ex::order_buy_type _pendingOrderBuyTypes)
{
    u256 _fromFee =
        0;  //(_toPendingOrderNum * _toPendingOrderPrice / PRICEPRECISION) / MATCHINGFEERATIO;
    u256 _toFee = 0;  //_toPendingOrderNum / MATCHINGFEERATIO;

    if (_pendingOrderType == order_type::buy && _pendingOrderTokenType == order_token_type::FUEL &&
            ...
            _pendingOrderBuyTypes == order_buy_type::all_price))
    {
        ...
        {
            _toFee =
                (_toPendingOrderNum * _toPendingOrderPrice / PRICEPRECISION) / MATCHINGFEERATIO;
        }
...
Json::Value State::queryExchangeReward(Address const& _address, unsigned _blockNum)
{
    ...
    if (_blockNum >= config::newChangeHeight())
    {
        do
        {
            ...
            {
                auto ret =
                    anytime_receivingPdFeeIncome(miner_mapping.first, (int64_t)_blockNum, false);
...
u256 State::rpcqueryBlcokReward(Address const& _address, unsigned _blockNum)
{
    ...
    u256 _voteNodeFee = 0;
    std::map<u256, std::map<Address, u256>>::const_iterator _voteDataIt =
        _votesnapshot.m_voteDataHistory.find(_numberofrounds - 1);
    ...
    for (; _voteDataIt != _votesnapshot.m_voteDataHistory.end(); _voteDataIt++)
    {
        ...
        std::map<u256, u256>::const_iterator _pollDataIt =
            _votesnapshot.m_pollNumHistory.find(_voteDataIt->first);
        ...
        if (_pollNum > 0)
        {
            ...
            if (_pollDataIt->first + 1 < _pair.first)
            {
                ...
                {
                    _pollFee =
                        _votesnapshot.m_blockSummaryHistory.find(_pollDataIt->first + 1)->second;
                }
            }
            else
            {
        ...
        for (auto _voteIt : _voteDataIt->second)
        {
            ...
            if (_pollNum > 0)
            {
                if (_voteDataIt->first + 1 < _pair.first)
                {
                    ...
                    {
                        _pollCookieFee =
                            _pollVoteSnapshot.m_blockSummaryHistory.find(_voteDataIt->first + 1)
                                ->second;
                    }
                }
                else
                ...
                {
                    _totalMap[_voteIt.first] =
                        std::pair<u256, u256>(_pollFeeIncome, _pollFeeIncome);
                }
                else
                {
        ...
        for (auto _totalIt : _totalMap)
        {
            ...
            {
                std::map<Address, std::pair<u256, u256>> _addrReceivedCookie =
                    _receivedCookies.m_received_cookies[_voteDataIt->first + 1];
...
void State::initMinerGasPrice(BlockHeader const& _header)
{
    ...
    {
        // u256 init_value = config::chainId() == MAINCHAINID ? 5 : config::chainId() == TESTCHAINID
        // ? 6 : 7;
        u256 init_value = 10;
...
Json::Value State::successPendingOrderForAddrMsg(
    dev::Address _a, int64_t _minTime, int64_t _maxTime, uint32_t _maxSize)
{
    ExdbState _exdbState(*this);
    _maxSize = _maxSize >= LIMITE_NUMBER ? LIMITE_NUMBER : _maxSize;
    std::vector<result_order> _retV =
        _exdbState.get_result_orders_by_address(_a, _minTime, _maxTime, _maxSize);
    Json::Value _JsArray;
...
void dev::brc::State::cancelPendingOrders(
    std::vector<std::shared_ptr<transationTool::operation>> const& _ops)
{
    ctrace << "cancle pendingorder";
    ex::order val;
    ...
    for (auto const& val : _ops)
    {
        std::shared_ptr<transationTool::cancelPendingorder_operation> can_pen =
            std::dynamic_pointer_cast<transationTool::cancelPendingorder_operation>(val);
        if (!can_pen)
        {
...
void State::subCookieIncomeNum(const dev::Address& _addr, const dev::u256& _value)
{
    ...
    if (!a || a->CookieIncome())
    {
        BOOST_THROW_EXCEPTION(NotEnoughCash() << errinfo_comment(std::string(
                                  "Account does not exist or account CookieIncome is too low ")));
    }
...
std::unordered_map<Address, u256> State::incomeSummary(
    const dev::Address& _addr, uint32_t _snapshotNum)
{
    if (auto a = account(_addr))
    {
...
void State::receivingIncome(const dev::Address& _addr,
    std::vector<std::shared_ptr<transationTool::operation>> const& _ops, int64_t _blockNum)
{
    trySnapshotWithMinerMapping(_addr, _blockNum);
    for (auto _it : _ops)
    {
        std::shared_ptr<transationTool::receivingincome_operation> _op =
            std::dynamic_pointer_cast<transationTool::receivingincome_operation>(_it);
        if (!_op)
        {
...
//
//    std::map<u256, std::map<Address, u256>>::iterator _voteDataIt =
//    _voteSnapshot.m_voteDataHistory.find(0 + 1); std::map<u256, u256>::iterator _pollDataIt =
//    _voteSnapshot.m_pollNumHistory.find(_numberofrounds + 1);      //first->rounds second->polls
//
//    if(_voteDataIt == _voteSnapshot.m_voteDataHistory.end() && _pollDataIt ==
//    _voteSnapshot.m_pollNumHistory.end())
//    {
//        BOOST_THROW_EXCEPTION(receivingincomeFiled() << errinfo_comment("There is currently no
//        income to receive"));
//    }
//
//    for(; _pollDataIt != _voteSnapshot.m_pollNumHistory.end(); _pollDataIt++)
//    {
//        auto _ownedHandingfee = _voteSnapshot.m_blockSummaryHistory.find(_pollDataIt->first + 1);
//        // first->rounds second->summaryCooike if (_pollDataIt->second <= 0 || _ownedHandingfee ==
//        _voteSnapshot.m_blockSummaryHistory.end())
//            continue;
//        _income += _ownedHandingfee->second - (_ownedHandingfee->second / 2 / _pollDataIt->second)
//        * _pollDataIt->second;
//    }
//    for(; _voteDataIt != _voteSnapshot.m_voteDataHistory.end(); _voteDataIt++)
//    {
...
//                auto _pollMap =
//                _pollAccountvoteSnapshot.m_pollNumHistory.find(_voteDataIt->first); auto
//                _handingfeeMap =
//                _pollAccountvoteSnapshot.m_blockSummaryHistory.find(_voteDataIt->first + 1);
//                if(_pollMap == _pollAccountvoteSnapshot.m_pollNumHistory.end() || _handingfeeMap
//                == _pollAccountvoteSnapshot.m_blockSummaryHistory.end())
//                    continue;
//                u256 _pollNum = 0;
//                u256 _handingfee = 0;
...
void State::receivingPdFeeIncome(const dev::Address& _addr, int64_t _blockNum)
{
    ...
    std::map<u256, std::map<Address, u256>>::const_iterator _voteDataIt =
        _voteSnapshot.m_voteDataHistory.find(_numofRounds + 1);
    u256 _BrcIncome = 0;
    u256 _CookieIncome = 0;
    for (; _voteDataIt != _voteSnapshot.m_voteDataHistory.end(); _voteDataIt++)
    {
        std::map<u256, std::vector<PollData>>::const_iterator _it =
            _couplingSystemfee.m_sorted_creaters.find(_voteDataIt->first);
        std::map<u256, std::pair<u256, u256>>::const_iterator _amountIt =
            _couplingSystemfee.m_Feesnapshot.find(_voteDataIt->first + 1);
        ...
        for (auto const& _voteAddressIt : _voteDataIt->second)
        {
            ...
            {
                std::map<Address, u256>::const_iterator _addrPollIt =
                    _superNodeAddrMap.find(_voteAddressIt.first);
                ...
                _CookieIncome +=
                    (_currentNodeCookie / 2 / _addrPollIt->second * _voteAddressIt.second);
            }
        }
    }
...
std::pair<u256, u256> State::anytime_receivingPdFeeIncome(
    const dev::Address& _addr, int64_t _blockNum, bool _is_update /*true*/)
{
    ...
    {
        BOOST_THROW_EXCEPTION(
            UnknownAccount() << errinfo_comment(std::string("can't get Account in received fee")));
    }
    ...
    for (auto const& val : vote_data(SysVarlitorAddress))
    {
        if (num)
        {
            ...
            if (_blockNum >= config::newChangeHeight())
            {
                ...
                {
                    if (miner->mappingAddress().first != Address() &&
                        miner->mappingAddress().second == val.m_addr)
                    {
    ...
    for (int i = start_round; i <= _pair.first; i++)
    {
        ...
        do
        {
            ...
            else
            {
                ...
                if (_blockNum >= config::newChangeHeight())
                {
                    for (int j = 0; j < config::minner_rank_num() &&
                                    j < system_sanp.m_sorted_creaters[i].size();
                         j++)
                    {
            ...
            for (auto const& val : check_creater)
            {
                ...
                if (_addr == val.m_addr)
                {
                    // super
                    _income_brcs +=
                        node_summary_brcs - (node_summary_brcs / 2 / val.m_poll * val.m_poll);
                    _income_cookies +=
                        node_summary_cookies - (node_summary_cookies / 2 / val.m_poll * val.m_poll);
                }
                ...
                fee_temp.up_received_cookies_brcs(
                    i, val.m_addr, std::make_pair(_income_brcs, _income_cookies), summary);
...
void State::transferAutoEx(std::vector<std::shared_ptr<transationTool::operation>> const& _ops,
    h256 const& _trxid, int64_t _timeStamp, u256 const& _baseGas, int64_t const& _blockNum)
{
    for (auto const& val : _ops)
    {
        std::shared_ptr<transationTool::transferAutoEx_operation> _op =
            std::dynamic_pointer_cast<transationTool::transferAutoEx_operation>(val);
        std::shared_ptr<transationTool::pendingorder_opearaion> const& _pdop =
            std::make_shared<transationTool::pendingorder_opearaion>((transationTool::op_type)3,
                _op->m_from, ex::order_type::buy, ex::order_token_type::FUEL,
                ex::order_buy_type::all_price, u256(0), _op->m_autoExNum);
        std::vector<std::shared_ptr<transationTool::operation>> _pdops;
        _pdops.push_back(_pdop);
        std::pair<u256, u256> _exNumPair = pendingOrders(_op->m_from, _timeStamp, _trxid, _pdops,
            _blockNum);  // first: exCookieNum  second:exBRCNum
...
void State::modifyGasPrice(std::vector<std::shared_ptr<transationTool::operation>> const& _ops)
{
    ...
    {
        std::shared_ptr<transationTool::modifyMinerGasPrice_operation> _op =
            std::dynamic_pointer_cast<transationTool::modifyMinerGasPrice_operation>(_it);
...
void State::changeMinerModifyGasPrice(
    std::vector<std::shared_ptr<transationTool::operation>> const& _ops)
{
    for (auto _it : _ops)
    {
        std::shared_ptr<transationTool::changeMiner_operation> _op =
            std::dynamic_pointer_cast<transationTool::changeMiner_operation>(_it);
        if (!_op)
        {
            BOOST_THROW_EXCEPTION(ChangeMinerFailed() << errinfo_comment(
                                      "changeMinerModifyGasPrice operation is null"));
        }
        ...
        {
            BOOST_THROW_EXCEPTION(ChangeMinerFailed() << errinfo_comment(
                                      "changeMinerModifyGasPrice minerGaspriceAddr is null"));
        }
...
void State::rollback(size_t _savepoint)
{
    while (_savepoint != m_changeLog.size())
    {
        ...
        cdebug << BrcYellow "rollback: "
               ...
               << " address " << change.address << BrcReset << std::endl;
...
std::pair<ExecutionResult, TransactionReceipt> State::execute(EnvInfo const& _envInfo,
    ...
    transationTool::initializeEnum _enum)
{
...
void dev::brc::State::execute_vote(Address const& _addr,
    std::vector<std::shared_ptr<transationTool::operation>> const& _ops, EnvInfo const& info)
{
    u256 block_num = (u256)info.number();
    for (auto const& val : _ops)
    {
        std::shared_ptr<transationTool::vote_operation> _p =
            std::dynamic_pointer_cast<transationTool::vote_operation>(val);
        if (!_p)
        {
...
Json::Value dev::brc::State::blockRewardMessage(
    Address const& _addr, uint32_t const& _pageNum, uint32_t const& _listNum)
{
    try
    {
        Json::Value jv;
        if (auto a = account(_addr))
        {
            if (a->blockReward().size() > 0)
            {
                ...
                {
                    std::copy(_Vector.begin() + (_pageNum - 1) * _listNum,
                        _Vector.begin() + _retList, res.begin());
                }
...
accountStu dev::brc::State::accountMsg(Address const& _addr)
{
    if (auto a = account(_addr))
    {
        ...
        accountStu _accountStu(_addr, a->balance(), a->FBalance(), a->BRC(), a->FBRC(),
            a->voteAll(), a->ballot(), a->poll(), a->nonce(), a->CookieIncome(), _vote);
        return _accountStu;
    }
    return accountStu();
}
...
std::vector<exchange_order> dev::brc::State::pendingorderPoolMsgV2(
    uint8_t _order_type, uint8_t _order_token_type, u256 getSize)
{
    ExdbState _exdbState(*this);
...
Account dev::brc::State::systemPendingorder(int64_t _time)
{
    ...
    ex_order _order = {h256(1), dev::systemAddress, u256Safe(std::string("100000000")),
        systenCookie, systenCookie, _time, dev::brc::ex::order_type::sell,
        dev::brc::ex::order_token_type::FUEL, dev::brc::ex::order_buy_type::only_price};
...
void dev::brc::State::add_vote(const dev::Address& _id, dev::brc::PollData const& p_data)
{
    ...
    if (a && rec_a && sys_a)
    {
        ...
        sys_a->set_system_poll(
            {_recivedAddr, rec_a->poll(), poll_data.m_time > 0 ? poll_data.m_time : p_data.m_time});
    }
    else
...
const PollData dev::brc::State::poll_data(
    const dev::Address& _addr, const dev::Address& _recv_addr) const
{
    if (auto a = account(_addr))
    {
...
void dev::brc::State::trySnapshotWithMinerMapping(const dev::Address& _addr, dev::u256 _block_num)
{
    ...
    {
        auto miner_mapping = minerMapping(_addr);
        if (miner_mapping.first != Address() && miner_mapping.first != _addr &&
            miner_mapping.second == _addr)
        {
...
void dev::brc::State::tryRecordFeeSnapshot(int64_t _blockNum)
{
    ...
    if (_pair.first > _rounds)
    {
        ...
        if (_rounds != 0 &&
            (ret_fee == a->getFeeSnapshot().m_sorted_creaters.end() || ret_fee->second.empty()))
        {
            return;
        }
        ...
        a->tryRecordSnapshot(_pair.first, a->BRC() - remainder_brc,
            a->balance() - remainder_ballance, vote_datas, _blockNum);
...
void dev::brc::State::set_last_block_record(
    const dev::Address& _id, std::pair<int64_t, int64_t> value, uint32_t varlitor_time)
{
    auto a = account(SysBlockCreateRecordAddress);
    if (!a)
    {
...
void dev::brc::State::try_newrounds_count_vote(
    const dev::brc::BlockHeader& curr_header, const dev::brc::BlockHeader& previous_header)
{
    // testlog << "curr:"<< curr_header.number() << "  pre:"<< previous_header.number();
    std::pair<uint32_t, Votingstage> previous_pair =
        dev::brc::config::getVotingCycle(previous_header.number());
    std::pair<uint32_t, Votingstage> curr_pair =
        dev::brc::config::getVotingCycle(curr_header.number());
    ...
    {
        m_changeLog.emplace_back(
            Change::MinnerSnapshot, SysVarlitorAddress, varlitor_a->vote_data());
        m_changeLog.emplace_back(
            Change::MinnerSnapshot, SysCanlitorAddress, standby_a->vote_data());
...
void dev::brc::State::addExchangeOrder(Address const& _addr, dev::brc::ex::ex_order const& _order)
{
    ...
    if (!_account)
    {
        BOOST_THROW_EXCEPTION(ExdbChangeFailed() << errinfo_comment(
                                  std::string("addExchangeOrder failed: account is not exist")));
    }
...
void dev::brc::State::removeExchangeOrder(const dev::Address& _addr, dev::h256 _trid)
{
    ...
    {
        BOOST_THROW_EXCEPTION(ExdbChangeFailed() << errinfo_comment(
                                  std::string("removeExchangeOrder failed: account is not exist")));
    }
    dev::brc::ex::ExOrderMulti _oldMulti = _account->getExOrder();
    ...
    if (status == false)
    {
        BOOST_THROW_EXCEPTION(ExdbChangeFailed() << errinfo_comment(std::string(
                                  "removeExchangeOrder failed: cancelpendingorder error")));
    }
    //  TO DO
    m_changeLog.emplace_back(Change::UpExOrder, _addr, _oldMulti);
}
...
dev::brc::ex::ExOrderMulti const& dev::brc::State::userGetExOrder(Address const& _addr)
{
    ...
    {
        BOOST_THROW_EXCEPTION(ExdbChangeFailed() << errinfo_comment(
                                  std::string("userGetExOrder failed : account is not exist")));
    }
...
void dev::brc::State::tryChangeMiner(
    const dev::brc::BlockHeader& curr_header, ChainParams const& params)
{
    ...
    if (!change_ret.first)
        return;
    changeMinerMigrationData(
        Address(change_ret.second.before_addr), Address(change_ret.second.new_addr), params);
}

void dev::brc::State::changeMinerAddVote(BlockHeader const& _header)
{
    //     if(_header.number() == 4756444){
    //         std::vector<std::tuple<std::string, std::string, std::string>> _changeVote =
    //         changeVote::getChangeMinerAddVote(); for (auto const& it : _changeVote) {
    ...
    //                 _a->addVote(std::pair<Address, u256>(Address(std::get<1>(it)),
    //                 u256(std::get<2>(it))));
...
void dev::brc::State::changeMinerMigrationData(
    const dev::Address& before_addr, const dev::Address& new_addr, ChainParams const& params)
{
...
void dev::brc::State::testCalculateMasterNodeDividend(
    std::map<Address, u256> const& _masterNodeVotes,
    std::map<Address, u256> const& _masterNodeDividend, u256 const& _totalVotes)
{
    for (auto const& _node : _masterNodeDividend)
    {
        ...
        {
            addBalance(
                _node.first, _node.second - (_node.second / 2 / _masterNodeVotes.at(_node.first) *
                                                _masterNodeVotes.at(_node.first)));
        }
    }
}


void dev::brc::State::testVotingAccountDividend(std::set<Address> const& _voteAddrs,
    std::map<Address, u256> const& _masterNodeDividends,
    std::map<Address, u256> const& _masterNodeVotes, int64_t const& _blockNum)
{
    for (auto const& _voteAddr : _voteAddrs)
    {
        ...
        {
            if (_masterNodeDividends.count(_voteInfo.first) &&
                _masterNodeVotes.count(_voteInfo.first))
            {
...
void dev::brc::State::dividend(std::set<Address> const& _dividendAddrs, int64_t _blockNum)
{
    ...
    std::map<Address, std::pair<u256, u256>> _superNode;  // Address : superNode Address, pair.first
                                                          // : Total votes, pair.second : Total
                                                          // dividend
    u256 _allMinerVotes;
...
void dev::brc::State::calculateMasterNodeDividend(
    std::map<Address, std::pair<u256, u256>>& _masterNode, u256 const& _totalDividendAmount,
    u256 const& _allMinerVotes)
{
    for (auto& _node : _masterNode)
    {
        ...
        addBalance(_node.first,
            (_nodeDividend - (_nodeDividend / 2 / _node.second.first * _node.second.first)));
    }
...
void dev::brc::State::votingAccountDividend(std::set<Address> const& _voteAddrs,
    std::map<Address, std::pair<u256, u256>> _masterNodeDividendInfo, int64_t _blockNum)
{
    for (auto const& _voteAddr : _voteAddrs)
    {
        try_new_vote_snapshot(_voteAddr, _blockNum);
        ...
        for (auto const& _voteInfo : _voteInfos)
        {
            if (_masterNodeDividendInfo.count(_voteInfo.first))
            {
                ...
                u256 _dividendAmount =
                    _votesAndDividend.second / 2 / _votesAndDividend.first * _voteInfo.second;
...
{
    ExdbState _exdbstate(*this);
    std::vector<exchange_order> _exchangeOrder =
        _exdbstate.get_order_by_type(order_type::buy, order_token_type::FUEL, LIMITE_NUMBER);
...
dev::brc::ex::ExResultOrder const& dev::brc::State::getSuccessExchange()
{
    ...
    {
        BOOST_THROW_EXCEPTION(
            SuccessExChangeFailed() << errinfo_comment("getSuccessExchange failed"));
    }
...
template <class DB>
AddressHash dev::brc::commit(
    AccountMap const& _cache, SecureTrieDB<Address, DB>& _state, int64_t _block_number /*= 0*/)
{
            ...
            else
            {
...

                {
                    ...
                    if (_block_number >= config::newChangeHeight())
                    {
                        ...
                        // cwarn << " insert rlp:" <<
                        // dev::toString(i.second.getRLPStreamChangeMiner());
                    }
                }
                {
                    /// fork about GasPrice
...
template AddressHash dev::brc::commit<StateCacheDB>(AccountMap const& _cache,
    SecureTrieDB<Address, StateCacheDB>& _state, int64_t _block_number = 0);
