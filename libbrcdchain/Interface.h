...
/**
 * @brief Main API hub for interfacing with BrcdChain.
 */
class Interface
{
public:
    ...
    // virtual h256 submitTransaction(TransactionSkeleton const& _t, Secret const& _secret, u256
    // _flag) = 0;
    ...
    ExecutionResult call(Secret const& _secret, u256 _value, Address _dest, bytes const& _data,
        u256 _gas, u256 _gasPrice, BlockNumber _blockNumber, FudgeFactor _ff = FudgeFactor::Strict)
    {
        return call(toAddress(_secret), _value, _dest, _data, _gas, _gasPrice, _blockNumber, _ff);
    }
    ExecutionResult call(Secret const& _secret, u256 _value, Address _dest, bytes const& _data,
    ...
    virtual Json::Value testBplusGet(u256 const& key, int64_t time, BlockNumber _block) const = 0;
    // [LOGS API]
    ...
    /// @returns the BRCSchedule in the context of the pending block.
    virtual BRCSchedule brcSchedule() const { return BRCSchedule(); }
    ...
    /// NOTE: This only works when compiled with BRC_FATDB; otherwise will throw
...
namespace std
{
void swap(dev::brc::Watch& _a, dev::brc::Watch& _b);
}
...
namespace std
{
inline void swap(dev::brc::Watch& _a, dev::brc::Watch& _b)
{
    swap(_a.m_c, _b.m_c);
