...
{
public:
    BrcdChainPeer() = default;
    BrcdChainPeer(std::shared_ptr<p2p::CapabilityHostFace> _host, NodeID const& _peerID,
    ...
    void debugSize() const{
        cnote << "peer ...";
        cnote << "m_knownBlocks:" << m_knownBlocks.size();
        cnote << "m_knownTransactions:" << m_knownTransactions.size();
    }
