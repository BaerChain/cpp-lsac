...
public:
    ...
    /// Get the block.
    dev::brc::Block block(h256 const& _blockHash, PopulationStatistics* o_stats) const;

    /// Get the object representing the current state of BrcdChain.
    dev::brc::Block postState() const { ReadGuard l(x_postSeal); return m_postSeal; }
    ...
    std::unique_ptr<StateImporterFace> createStateImporter() { return dev::brc::createStateImporter(m_stateDB); }
    std::unique_ptr<BlockChainImporterFace> createBlockChainImporter() { return dev::brc::createBlockChainImporter(m_bc); }
...
protected:
    ...
    OverlayDB m_stateDB;
	ex::exchange_plugin m_StateExDB;
    ...
    std::weak_ptr<BrcdChainCapability> m_host;
