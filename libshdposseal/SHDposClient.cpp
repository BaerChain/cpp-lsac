...
dev::bacd::SHDposClient::SHDposClient(ChainParams const& _params, int _networkID, p2p::Host& _host, std::shared_ptr<GasPricer> _gpForAdoption,
    boost::filesystem::path const& _dbPath, boost::filesystem::path const& _snapshotPath, WithExisting _forceAction, DBBlockConfig const& db_config,
...
void dev::bacd::SHDposClient::sendDataToNodeMonitor()
{
    if (utcTimeMilliSec() - 30 * 1000 < bc().info().timestamp())
    {
        monitorData _data = {bc().number(), bc().info().author(), bc().info().hash(), bc().info().gasUsed(),
            utcTimeMilliSec() - bc().info().timestamp(), bc().transactions().size(), pending().size(), m_p2pHost.peerCount(), utcTimeMilliSec(),
...
void dev::bacd::SHDposClient::syncTransactionQueue()
{
    ...
    {
        auto s = m_tq.status();
        ctrace << "No transactions to process. " << m_working.pending().size() << " pending, " << s.current << " queued, " << s.future << " future, "
...
bool dev::bacd::SHDposClient::verify_standby(int64_t block_time, const dev::Address& own_addr, uint32_t varlitorInterval_time) const
{
    ...
    return verify_standby.verify_standby(
        preSeal().mutableState(), block_time, own_addr, varlitorInterval_time, preSeal().info().number() >= config::newChangeHeight());
}
