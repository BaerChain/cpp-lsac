...
bool dev::bacd::SHDpos::isCurrBlock(Address const &_curr) {
    ...
    if (iter != m_punishVarlitor.end()) {
        if (iter->second.m_isPunish &&
            (iter->second.m_badBlockNum >= BadNum_Two && iter->second.m_badBlockNum < BadNum_Three)) {
            return true;
        }
    }
    return false;
...
size_t dev::bacd::SHDpos::kickoutVarlitors() {
    ...
    for (auto iter = m_punishVarlitor.begin(); iter != m_punishVarlitor.end(); iter++) {
        if (iter->second.m_isPunish &&
            (iter->second.m_badBlockNum >= BadNum_One && iter->second.m_badBlockNum < BadNum_Two)) {
